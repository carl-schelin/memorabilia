    ...
    if (SaveIndex == 0) {
        ...
        if ((input = fopen("Dot2Dot.Dat", "r")) != NULL)
            while (!feof(input)) {
                ...
                if (temp[0] == '2') {
                    sscanf(temp, "%d %d %d", &recNum, &matCol, &matRow);
                    if (recNum == 200) {   /* horizontal line */
                        pCol = matCol * 28 + BoxBCol + 8;
                        pRow = matRow * 28 + BoxBRow + 3;
                    }
                    if (recNum == 201) {   /* vertical line */
                        pCol = matCol * 28 + BoxBCol + 3;
                        pRow = matRow * 28 + BoxBRow + 8;
                    }
