        ...
        if (postrow + 24 < BrdERow)
            if (!Depressed(postcol, postrow, VERT))
                if (postcol > 0 && postrow > 0) {
                    sprintf(GameMoves[SaveIndex++], "s%3d %3d", col, row);
                    NewPlr = TRUE;
                    MouRemovePtr();
...
int Depressed(int col, int row, int toggle)
{
    ...
    if (toggle) {
        if (getpixel(col, row + 12) == DARKGRAY && getpixel(col + 4, row + 12) == WHITE)
            yep = TRUE;
    }
    else {
        if (getpixel(col + 12, row) == DARKGRAY && getpixel(col + 12, row + 4) == WHITE)
            yep = TRUE;
    }
    MouDrawPtr();
    return yep;
