...
int QueBCol, QueBRow, QueECol, QueERow;
...
int Are_You_Sure(void)
{
    ...
    if (Mouse)
        MouRemovePtr();
    size = imagesize(QueBCol - 1, QueBRow - 1, QueECol + 6, QueERow + 6);
    buffer = malloc(size);
    getimage(QueBCol - 1, QueBRow - 1, QueECol + 6, QueERow + 6, buffer);
    Draw_Button(QueBCol, QueBRow, QueECol, QueERow, "Are you sure?", QueBRow + 8, 1, DARKGRAY, RED, WHITE);
    Draw_Button(QueBCol + 11, QueBRow + 25, QueBCol + 56, QueBRow + 40, "Yes", QueBRow + 29, 0, DARKGRAY, LIGHTGRAY, RED);
    Draw_Button(QueECol - 57, QueBRow + 25, QueECol - 12, QueBRow + 40,  "No", QueBRow + 29, 0, DARKGRAY, LIGHTGRAY, RED);
    if (Mouse) {
        MouDrawPtr();
    ...
    if (Mouse) 
        MouRemovePtr();
    putimage(QueBCol - 1, QueBRow - 1, buffer, COPY_PUT);
    free(buffer);
    if (Mouse)
        MouDrawPtr();
    return ret_Val;
